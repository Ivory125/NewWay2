...
const float c_fDefaultHorseRotationSpeed = 1200.0f;
...
void CInstanceBase::SetPetScale(int iLevel)
{
	...
	for (int i = 0; i < TABLE_LEN; ++i) {
		if (iLevel >= PetScaleTable[i].dwMinLevel && iLevel <= PetScaleTable[i].dwMaxLevel) {
...
BYTE CInstanceBase::GetElementEffect()
{
	...
	return static_cast<BYTE>(m_dwElementsEffect);
}
