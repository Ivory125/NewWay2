...
bool CPythonItem::IsIgnoredItem(DWORD dwVirtualNumber)
{
	...
	bool ignore_pickup_weapon = CPythonSystem::Instance().GetPickupIgnore(CPythonSystem::PICKUP_IGNORE_WEAPON);
	bool ignore_pickup_armor = CPythonSystem::Instance().GetPickupIgnore(CPythonSystem::PICKUP_IGNORE_ARMOR);
	bool ignore_pickup_head = CPythonSystem::Instance().GetPickupIgnore(CPythonSystem::PICKUP_IGNORE_HEAD);
	bool ignore_pickup_shield = CPythonSystem::Instance().GetPickupIgnore(CPythonSystem::PICKUP_IGNORE_SHIELD);
	bool ignore_pickup_wrist = CPythonSystem::Instance().GetPickupIgnore(CPythonSystem::PICKUP_IGNORE_WRIST);
	bool ignore_pickup_foots = CPythonSystem::Instance().GetPickupIgnore(CPythonSystem::PICKUP_IGNORE_FOOTS);
	bool ignore_pickup_neck = CPythonSystem::Instance().GetPickupIgnore(CPythonSystem::PICKUP_IGNORE_NECK);
	bool ignore_pickup_ear = CPythonSystem::Instance().GetPickupIgnore(CPythonSystem::PICKUP_IGNORE_EAR);
	bool ignore_pickup_etc = CPythonSystem::Instance().GetPickupIgnore(CPythonSystem::PICKUP_IGNORE_ETC);

	if (pkItemData->GetType() == CItemData::ITEM_TYPE_WEAPON && ignore_pickup_weapon)
	{
		return true;
	}
	...
	if ((pkItemData->GetType() != CItemData::ITEM_TYPE_WEAPON && pkItemData->GetType() != CItemData::ITEM_TYPE_ARMOR) && ignore_pickup_etc)
	{
		return true;
	}
...
std::vector<DWORD> CPythonItem::GetCloseMoneys(const TPixelPosition& c_rPixelPosition, DWORD dwDistance)
{
