...
PyObject* netSendBornPet(PyObject* poSelf, PyObject* poArgs)
{
	char* petname;

	if (!PyTuple_GetString(poArgs, 0, &petname))
		return Py_BuildException();

	if (strlen(petname) > 12 || strlen(petname) < 4) {
		TraceError("SendBornPetName: petname lenght not allowed!");
		return Py_BuildNone();
	}

	CPythonNetworkStream::Instance().SendBornPet(petname);
	return Py_BuildNone();
}

PyObject* netSendRenewPetName(PyObject* poSelf, PyObject* poArgs)
{
	char* petname;
	int itempos1;
	int itempos2;
	...
	if (!PyTuple_GetInteger(poArgs, 1, &itempos1))
		return Py_BuildException();

	if (!PyTuple_GetInteger(poArgs, 2, &itempos2))
		return Py_BuildException();

	if (strlen(petname) > 12 || strlen(petname) < 4) {
		TraceError("SendRenewPetName: petname lenght not allowed!");
		return Py_BuildNone();
	}

	CPythonNetworkStream::Instance().SendRenewPetName(petname, itempos1, itempos2);
	return Py_BuildNone();
}
#endif
...
#endif
#ifdef ENABLE_ASLAN_GROWTH_PET_SYSTEM
		{ "RequestBornPet",							netSendBornPet,								METH_VARARGS },
		{ "RequestRenewPetName",					netSendRenewPetName,						METH_VARARGS },
#endif
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
