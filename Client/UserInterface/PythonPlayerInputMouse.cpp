...
bool CPythonPlayer::AutoFarmQuqueSet(const bool isAdd, const DWORD dwVirtualID)
{
	if (isAdd)
	{
		...
		{
			CPythonChat::Instance().AppendChat(1, "[Metin Farm] Target removed in quque.");
			AutoFarmQuqueSet(false, dwVirtualID);
			return false;
		}
		else if (m_vecQuqueAutoAttack.size() + 1 > bTotalQuqueAutoAttack)
		{
			CPythonChat::Instance().AppendChat(1, "[Metin Farm] Quque target list is max.");
			return false;
		}

		CPythonChat::Instance().AppendChat(1, "[Metin Farm] Target added in quque.");
