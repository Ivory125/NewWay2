...
PyObject * playerGetItemLink(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (pPlayerItem && CItemManager::Instance().GetItemDataPointer(pPlayerItem->vnum, &pItemData))
	{
		...
		len += snprintf(itemlink + len, sizeof(itemlink) - len, ":%x:%x",
				pPlayerItem->grade_element, pPlayerItem->element_type_bonus);


		if(pPlayerItem->grade_element > 0)
		{
			len += snprintf(itemlink + len, sizeof(itemlink) - len, ":%x:%x",
				pPlayerItem->attack_element[pPlayerItem->grade_element-1], pPlayerItem->elements_value_bonus[pPlayerItem->grade_element-1]);
		}
		else{
			len += snprintf(itemlink + len, sizeof(itemlink) - len, ":%x:%x",
				0,0);
...
PyObject * playerElements(PyObject * poSelf, PyObject * poArgs)
{
	...
	{
		return Py_BuildValue("i", ELEMENT_CANT_WORLDARD);
	}
	...
	DWORD dwSlotIndexItemWeapon = CPythonPlayer::Instance().GetItemIndex(SlotIndexItemWeapon);
	...
	{
		BYTE dwGradeItem = CPythonPlayer::Instance().GetElementGrade(SlotIndexItemWeapon);

		BYTE byType_Item_Weapon = CPythonPlayer::Instance().GetElementType(SlotIndexItemWeapon);
...
PyObject * playerGetItemElementGrade(PyObject* poSelf, PyObject* poArgs)
{
	switch (PyTuple_Size(poArgs))
	{
		case 1:
		{
			...
			DWORD dwElementGrade = CPythonPlayer::Instance().GetElementGrade(TItemPos(INVENTORY, iSlotIndex));
		...
		case 2:
		{
			...
			DWORD dwElementGrade = CPythonPlayer::Instance().GetElementGrade(Cell);
			return Py_BuildValue("i", dwElementGrade);
		}
		default:
			return Py_BuildException();
...
PyObject * playerGetItemElementAttack(PyObject* poSelf, PyObject* poArgs)
{
	...
	switch (PyTuple_Size(poArgs))
	{
		case 2:
		{
			...
			DWORD dwElementAttack = CPythonPlayer::Instance().GetElementAttack(TItemPos(INVENTORY, iSlotIndex),iAttackIndex);
		...
		case 3:
		{
			...
			DWORD dwElementAttack = CPythonPlayer::Instance().GetElementAttack(Cell,iAttackIndex);
			return Py_BuildValue("i", dwElementAttack);
		}
		default:
			return Py_BuildException();
...
PyObject * playerGetItemElementType(PyObject* poSelf, PyObject* poArgs)
{
	switch (PyTuple_Size(poArgs))
	{
		case 1:
		{
			...
			BYTE dwElementType = CPythonPlayer::Instance().GetElementType(TItemPos(INVENTORY, iSlotIndex));
		...
		case 2:
		{
			...
			BYTE dwElementType = CPythonPlayer::Instance().GetElementType(Cell);
			return Py_BuildValue("i", dwElementType);
		}
		default:
			return Py_BuildException();
...
PyObject * playerGetItemElementValue(PyObject* poSelf, PyObject* poArgs)
{
	...
	switch (PyTuple_Size(poArgs))
	{
		case 2:
		{
			...
			short dwElementValue = CPythonPlayer::Instance().GetElementValue(TItemPos(INVENTORY, iSlotIndex),iValueIndex);
		...
		case 3:
		{
			...
			short dwElementValue = CPythonPlayer::Instance().GetElementValue(Cell,iValueIndex);
			return Py_BuildValue("i", dwElementValue);
		}
		default:
			return Py_BuildException();
...
void initPlayer()
{
	...
	// Party
 	...
 	PyModule_AddIntConstant(poModule, "ELEMENT_CANT_WORLDARD",				ELEMENT_CANT_WORLDARD);
	...
	PyModule_AddIntConstant(poModule, "ON_TOP_WND_PET_FEED_LIFE", ON_TOP_WND_PET_FEED_LIFE);
	PyModule_AddIntConstant(poModule, "ON_TOP_WND_PET_FEED_EQUIP", ON_TOP_WND_PET_FEED_EQUIP);
	PyModule_AddIntConstant(poModule, "ON_TOP_WND_PET_PAGE_ATTR", ON_TOP_WND_PET_PAGE_ATTR);
	PyModule_AddIntConstant(poModule, "ON_TOP_WND_PET_PAGE_REVIVE_1", ON_TOP_WND_PET_PAGE_REVIVE_1);
	PyModule_AddIntConstant(poModule, "ON_TOP_WND_PET_PAGE_REVIVE_2", ON_TOP_WND_PET_PAGE_REVIVE_2);
#endif
#endif
}
