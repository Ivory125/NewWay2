...
#ifdef ENABLE_EXTENDED_PICKUP
	m_Config.bPickupMode = PICKUP_MODE_SINGLE;
...
{
	if (bMode != PICKUP_MODE_SINGLE && bMode != PICKUP_MODE_ALL)
	{
		TraceError("SetPickupMode:: Invalid PickupMode %d (Single: %d - All: %d)", bMode, PICKUP_MODE_SINGLE, PICKUP_MODE_ALL);
		return;
	}
...
bool CPythonSystem::GetPickupIgnore(int iType)
{
	...
	{
		TraceError("GetPickupIgnore:: Invalid Ignoretype %d (0 - %d)", iType, PICKUP_IGNORE_MAX_NUM);
		return false;
	}
...
void CPythonSystem::SetPickupIgnore(int iType, int iIgnore)
{
	if (iType < 0 || iType >= PICKUP_IGNORE_MAX_NUM)
	{
		TraceError("SetPickupIgnore:: Invalid Ignoretype %d (0 - %d)", iType, PICKUP_IGNORE_MAX_NUM);
		return;
	}
...
#ifdef ENABLE_SHOW_MONEY_ON_OFF
bool CPythonSystem::GetMoneyOnOff()
{
...
bool CPythonSystem::LoadConfig()
{
	...
	while (fgets(buf, 256, fp))
	{
		...
		{
			m_Config.b_arPickupIgnore[PICKUP_IGNORE_WEAPON] = atoi(value) == 1 ? true : false;
		}
		else if (!stricmp(command, DEF_STR(PICKUP_IGNORE_ARMOR)))
		{
			m_Config.b_arPickupIgnore[PICKUP_IGNORE_ARMOR] = atoi(value) == 1 ? true : false;
		}
		else if (!stricmp(command, DEF_STR(PICKUP_IGNORE_HEAD)))
		{
			m_Config.b_arPickupIgnore[PICKUP_IGNORE_HEAD] = atoi(value) == 1 ? true : false;
		}
		else if (!stricmp(command, DEF_STR(PICKUP_IGNORE_SHIELD)))
		{
			m_Config.b_arPickupIgnore[PICKUP_IGNORE_SHIELD] = atoi(value) == 1 ? true : false;
		}
		else if (!stricmp(command, DEF_STR(PICKUP_IGNORE_WRIST)))
		{
			m_Config.b_arPickupIgnore[PICKUP_IGNORE_WRIST] = atoi(value) == 1 ? true : false;
		}
		else if (!stricmp(command, DEF_STR(PICKUP_IGNORE_FOOTS)))
		{
			m_Config.b_arPickupIgnore[PICKUP_IGNORE_FOOTS] = atoi(value) == 1 ? true : false;
		}
		else if (!stricmp(command, DEF_STR(PICKUP_IGNORE_NECK)))
		{
			m_Config.b_arPickupIgnore[PICKUP_IGNORE_NECK] = atoi(value) == 1 ? true : false;
		}
		else if (!stricmp(command, DEF_STR(PICKUP_IGNORE_EAR)))
		{
			m_Config.b_arPickupIgnore[PICKUP_IGNORE_EAR] = atoi(value) == 1 ? true : false;
		}
		else if (!stricmp(command, DEF_STR(PICKUP_IGNORE_ETC)))
		{
			m_Config.b_arPickupIgnore[PICKUP_IGNORE_ETC] = atoi(value) == 1 ? true : false;
		}
#endif
#ifdef ENABLE_SHOW_MONEY_ON_OFF
...
bool CPythonSystem::SaveConfig()
{
	...
	for (int i = 0; i < PICKUP_IGNORE_MAX_NUM; ++i)
	{
		...
		{
			DEF_STR(PICKUP_IGNORE_WEAPON),
			DEF_STR(PICKUP_IGNORE_ARMOR),
			DEF_STR(PICKUP_IGNORE_HEAD),
			DEF_STR(PICKUP_IGNORE_SHIELD),
			DEF_STR(PICKUP_IGNORE_WRIST),
			DEF_STR(PICKUP_IGNORE_FOOTS),
			DEF_STR(PICKUP_IGNORE_NECK),
			DEF_STR(PICKUP_IGNORE_EAR),
