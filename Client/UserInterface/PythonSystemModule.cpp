...
PyObject* systemSetPickupMode(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonSystem::Instance().SetPickupMode(bMode);
	return Py_BuildNone();
}

PyObject* systemGetPickupIgnore(PyObject* poSelf, PyObject* poArgs)
{
	int iType;
	...
	return Py_BuildValue("i", CPythonSystem::Instance().GetPickupIgnore(iType));
}

PyObject* systemSetPickupIgnore(PyObject* poSelf, PyObject* poArgs)
{
	int iType;
	if (!PyTuple_GetInteger(poArgs, 0, &iType))
	{
	...
	int iIgnore;
	if (!PyTuple_GetInteger(poArgs, 1, &iIgnore))
	{
		return Py_BuildException();
	}
	CPythonSystem::Instance().SetPickupIgnore(iType, iIgnore);
	return Py_BuildNone();
}
#endif
...
PyObject* systemGetMoneyOnOff(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("i", CPythonSystem::Instance().GetMoneyOnOff());
}
#endif
...
void initsystemSetting()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetPickupIgnore",			systemGetPickupIgnore,			METH_VARARGS },
		{ "SetPickupIgnore",			systemSetPickupIgnore,			METH_VARARGS },
#endif	
...
#endif
#ifdef ENABLE_SHOW_MONEY_ON_OFF
		{ "GetMoneyOnOff",				systemGetMoneyOnOff,			METH_VARARGS },
#endif
...
#ifdef ENABLE_EXTENDED_PICKUP
	PyModule_AddIntConstant(poModule, "PICKUP_MODE_SINGLE", CPythonSystem::PICKUP_MODE_SINGLE);
	PyModule_AddIntConstant(poModule, "PICKUP_MODE_ALL", CPythonSystem::PICKUP_MODE_ALL);
#endif
}
