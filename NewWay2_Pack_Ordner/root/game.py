...
class GameWindow(ui.ScriptWindow):
	...
	def __BuildKeyDict(self):
		...
		onPressKeyDict[app.DIK_F7]		= lambda : self.interface.BuffNPCOpenWindow()
		onPressKeyDict[app.DIK_F6]	= lambda : self.__toggleSwitchbot()
		onPressKeyDict[app.DIK_X]			= lambda : self.savekordi()
		onPressKeyDict[app.DIK_F9]			= lambda : self.opensetting()
		...
		if app.ENABLE_ASLAN_GROWTH_PET_SYSTEM:
			onClickKeyDict[app.DIK_P] = lambda: self.interface.TogglePetMain()
...
#		chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.GAME_PICK_MONEY % localeInfo.MoneyFormat(money)) # @fixme020
	def OnPickMoney(self, money):
		if systemSetting.GetMoneyOnOff() == True:
	...
	def __ServerCommand_Build(self):
		...
		if app.ENABLE_ASLAN_GROWTH_PET_SYSTEM:
			serverCommandList["GrowthPetSetSkillCooltime"]	= self.__GrowthPetSetSkillCooltime
			serverCommandList["GrowthPetGetType"] 			= self.__GrowthPetGetType
			serverCommandList["GrowthPetOpenIncubator"]		= self.__GrowthPetOpenIncubator
			serverCommandList["GrowthPetSetEvoInfo"]			= self.__GrowthPetSetEvoInfo
		...
		if app.ENABLE_ASLAN_MODULAR_ADMIN_PANEL:
			serverCommandList["AdminpanelGetPlayerCount"]	= self.__AdminpanelRecivePlayerCount
...
###############################################################################################################################ORIGINAL##########################
			ELEMENT_NAME = {
				item.APPLY_ENCHANT_ELECT : uiScriptLocale.REFINE_ELEMENT_CHANGE_ELECT,
				item.APPLY_ENCHANT_FIRE : uiScriptLocale.REFINE_ELEMENT_CHANGE_FIRE,
				item.APPLY_ENCHANT_ICE : uiScriptLocale.REFINE_ELEMENT_CHANGE_ICE,
				item.APPLY_ENCHANT_WIND : uiScriptLocale.REFINE_ELEMENT_CHANGE_WIND,
				item.APPLY_ENCHANT_EARTH : uiScriptLocale.REFINE_ELEMENT_CHANGE_EARTH,
				item.APPLY_ENCHANT_DARK : uiScriptLocale.REFINE_ELEMENT_CHANGE_DARK,
			}
	...
	def savekordi(self):
		(x, y, z) = player.GetMainCharacterPosition()
		if str(constInfo.regengenerator) == "0/0/0/0":

			with open('kordi.txt', 'a') as file:
				file.write("%.0f	%.0f\n" % (x/100, y/100))
		else:
			g = constInfo.regengenerator.split("/")
			with open('regen.txt', 'a') as file:
				file.write("%s	%.0f	%.0f	%d	%d	0	0	%s	100	1	%d\n" % (str(g[0]),x/100, y/100,int(g[1]),int(g[1]),str(g[2]),int(g[3])))
	...
	if app.ENABLE_ASLAN_MODULAR_ADMIN_PANEL:
		def __AdminpanelRecivePlayerCount(self, red, yellow, blue, total):
			if self.interface and self.interface.wndAdminPanel:
				self.interface.wndAdminPanel.windowList["startpage"].RecivePlayerCount(int(red), int(yellow), int(blue), int(total))

	if app.ENABLE_ASLAN_GROWTH_PET_SYSTEM:
		def BINARY_GrowthPetSetInfo(self, SealVnum, SealSlot, Name, Age, Birthday, SealDuration, BornDuration, Level, Evolution, CanEvoUpgrade, Exp, ExpItem, LevelExpMob, LevelExpItem, ValueATK, ValueHP, ValueDEF, Skill0, SkillLevel0, Skill1, SkillLevel1, Skill2, SkillLevel2):
			self.interface.GrowthPet_SetBasicInfo(int(SealVnum), int(SealSlot), Name, int(Age), int(Birthday), int(SealDuration), int(BornDuration))
			self.interface.GrowthPet_SetLevelInfo(int(Level), int(Evolution), int(CanEvoUpgrade), int(Exp), int(ExpItem), int(LevelExpMob), int(LevelExpItem))
			self.interface.GrowthPet_SetBonusInfo(int(ValueATK), int(ValueHP), int(ValueDEF))
			self.interface.GrowthPet_SetSkillInfo(int(Skill0), int(SkillLevel0), int(Skill1), int(SkillLevel1), int(Skill2), int(SkillLevel2))
			(constInfo.GROWTH_PET_SKILL_1, constInfo.GROWTH_PET_SKILL_2, constInfo.GROWTH_PET_SKILL_3) = (int(Skill0)+5400, int(Skill1)+5400, int(Skill2)+5400)
			self.affectShower.BINARY_NEW_AddAffect(constInfo.GROWTH_PET_SKILL_1, 1, 1, 0)
			self.affectShower.BINARY_NEW_AddAffect(constInfo.GROWTH_PET_SKILL_2, 1, 1, 0)
			self.affectShower.BINARY_NEW_AddAffect(constInfo.GROWTH_PET_SKILL_3, 1, 1, 0)
			
		def __GrowthPetSetSkillCooltime(self, SkillSlot, Cooltime):
			self.interface.GrowthPet_SetSkillCoolTime(int(SkillSlot), int(Cooltime))
		
		def BINARY_GrowthPetUnsummon(self):
			self.interface.GrowthPet_SetUnsummon()
			self.affectShower.BINARY_NEW_RemoveAffect(int(constInfo.GROWTH_PET_SKILL_1),0)
			self.affectShower.BINARY_NEW_RemoveAffect(int(constInfo.GROWTH_PET_SKILL_2),0)
			self.affectShower.BINARY_NEW_RemoveAffect(int(constInfo.GROWTH_PET_SKILL_3),0)
			(constInfo.GROWTH_PET_SKILL_1, constInfo.GROWTH_PET_SKILL_2, constInfo.GROWTH_PET_SKILL_3) = (0, 0, 0)

		def __GrowthPetGetType(self, window, type):
			self.interface.GrowthPet_GetType(int(window), int(type))
	
		def __GrowthPetOpenIncubator(self, type, seal, cost):
			self.interface.GrowthPet_SetIncubator(int(type), int(seal), int(cost))

		def __GrowthPetSetEvoInfo(self, open, vnum, count):
			if int(open) == 1:
				self.interface.GrowthPet_OpenEvoInfo()
			else:
				self.interface.GrowthPet_SetEvoInfoItem(int(vnum), int(count))