...
class ChestDropWindow(ui.ScriptWindow):
	...
	def SetOpenAmount(self, amount):
		...
		self.openChestButtonSingle.SetText("Truhe %d x öffnen" % self.openAmount)
		self.openChestButtonMultiple.SetText("Truhe %d x öffnen" % self.openAmount)
	...
	def RefreshItems(self, chestVnum):
		...
		if not self.chestDrop.has_key(self.currentChest):
			chat.AppendChat(chat.CHAT_TYPE_INFO, "Diese Truhe hat keinen Drop. Melden Sie Probleme einem Administrator.")
			self.Close()
			return
