...
bool CClientManager::InitializeIkarusShopShopItemsTable()
{
	...
	for(uint32_t i=0; i < msg->Get()->uiNumRows; i++)
	{
		...
		str_to_number(item.grade_element, row[col++]);
		str_to_number(item.attack_element[0], row[col++]);
		str_to_number(item.attack_element[1], row[col++]);
		str_to_number(item.attack_element[2], row[col++]);
		str_to_number(item.element_type_bonus, row[col++]);
		str_to_number(item.elements_value_bonus[0], row[col++]);
		str_to_number(item.elements_value_bonus[1], row[col++]);
		str_to_number(item.elements_value_bonus[2], row[col++]);
#endif
...
#ifdef ELEMENT_SPELL_WORLDARD
		str_to_number(info.grade_element, row[col++]);
		str_to_number(info.attack_element[0], row[col++]);
		str_to_number(info.attack_element[1], row[col++]);
		str_to_number(info.attack_element[2], row[col++]);
		str_to_number(info.element_type_bonus, row[col++]);
		str_to_number(info.elements_value_bonus[0], row[col++]);
		str_to_number(info.elements_value_bonus[1], row[col++]);
		str_to_number(info.elements_value_bonus[2], row[col++]);
#endif
#ifdef ENABLE_CHANGELOOK_SYSTEM
...
#endif
	//sitem.grade_element = pItem->GetElementGrade();
	//thecore_memcpy(sitem.attack_element, pItem->GetElementAttacks(), sizeof(sitem.attack_element));
	//sitem.element_type_bonus = pItem->GetElementsType();
	//thecore_memcpy(sitem.elements_value_bonus, pItem->GetElementsValues(), sizeof(sitem.elements_value_bonus));

	sys_err("ItemIka Elemental - Grade %d / Type %d", item.grade_element, item.element_type_bonus);
...
#ifdef ELEMENT_SPELL_WORLDARD
	query += "`grade_elements`, `attackelement1`, `attackelement2`, `attackelement3`, `elementstype`, `elementsvalue1`, `elementsvalue2`, `elementsvalue3`, ";
#endif
#ifdef ENABLE_CHANGELOOK_SYSTEM
	query += "`transmutation`, ";
#endif
...
#ifdef ELEMENT_SPELL_WORLDARD
	query += "i.`grade_elements`, i.`attackelement1`, i.`attackelement2`, i.`attackelement3`, i.`elementstype`, i.`elementsvalue1`, i.`elementsvalue2`, i.`elementsvalue3`, ";
#endif
#ifdef ENABLE_CHANGELOOK_SYSTEM
	query += "i.`trans`, ";
#endif
