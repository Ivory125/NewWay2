...
#ifdef ENABLE_ASLAN_BUFF_NPC_SYSTEM
#ifdef ENABLE_ASLAN_GROWTH_PET_SYSTEM
	if (IsPC() == true || m_bCharType == CHAR_TYPE_NPC || IsBuffNPC() == true || IsGrowthPet() == true)
#else
	if (IsPC() == true || m_bCharType == CHAR_TYPE_NPC || IsBuffNPC() == true)
#endif
#else
#ifdef ENABLE_ASLAN_GROWTH_PET_SYSTEM
	if (IsPC() == true || m_bCharType == CHAR_TYPE_NPC || IsGrowthPet() == true)
#else
...
#ifdef ENABLE_ASLAN_BUFF_NPC_SYSTEM
		else if (IsBuffNPC() == true)
		{
			addPacket.dwLevel = GetLevel();
		}
#endif
#ifdef ENABLE_ASLAN_GROWTH_PET_SYSTEM
		else if (IsGrowthPet() == true)
		{
			addPacket.dwLevel = GetLevel();
		}
#endif
...
//					AutoGiveItem(27003, 2);
		...
		case POINT_ATT_ELEMENTS:
			//SetPoint(type, GetPoint(type) + amount);-----TEST
			//val = GetPoint(type);-----TEST
			//break;-----TEST
#endif
...
{
	LPITEM seal = FindItemByID(GetQuestFlag("growth_pet.seal_id"));

	if (GetGrowthPetSystem() != NULL && seal != NULL) {
		if (seal->GetType() == ITEM_PET_ASLAN && seal->GetSubType() == PET_SEAL_ASLAN)
			GetGrowthPetSystem()->Summon(seal);
	}
}
...
DWORD CHARACTER::PetGetNextExp(BYTE type) const
{
	if (IsGrowthPet()) {
		if (GROWTH_PET_MAX_LEVEL < GetLevel())
			return 2500000000;
		else {
			if (type == 0)
				return pet_exp_table[GetLevel()].dwEXPMob;
			if (type == 1)
				return pet_exp_table[GetLevel()].dwEXPItem;
		}
	}
	return 0;
}
