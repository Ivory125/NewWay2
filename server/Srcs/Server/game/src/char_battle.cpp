...
void CHARACTER::RewardGold(LPCHARACTER pkAttacker)
{
	...
	if (GetMobRank() >= MOB_RANK_BOSS && !IsStone() && GetMobTable().dwGoldMax != 0)
	{
		...
		for (int i = 0; i < iSplitCount; ++i)
		{
			...
			{
				pos.x = GetX() + ((number(-14, 14) + number(-14, 14)) * 10);//Kleiner test von der Positio (Original 23 beide )
				pos.y = GetY() + ((number(-14, 14) + number(-14, 14)) * 10);//wie weit entfernt das yang gedroppt wird :) 
	...
	else if (1 == number(1, iGold10DropPct))
	{
		for (int i = 0; i < 10; ++i)
		{
			...
			if ((item = ITEM_MANAGER::instance().CreateItem(1, iGold)))
			{
				pos.x = GetX() + (number(-7, 7) * 10);//hier das gleiche Original 20
				pos.y = GetY() + (number(-7, 7) * 10);
	...
	else
	{
		...
		if (iGold != 0)
		{
			...
			for (int i = 0; i < iSplitCount; ++i)
			{
				...
				{
					pos.x = GetX() + (number(-7, 7) * 10);// und hier 20
					pos.y = GetY() + (number(-7, 7) * 10);
...
{
#ifdef ENABLE_ASLAN_GROWTH_PET_SYSTEM
	int iPetExp = CALCULATE_VALUE_LVDELTA(to->GetGrowthPetSystem()->GetLevel(), from->GetLevel(), iExp);

	if (to->IsEquipUniqueItem(NEW_PET_EXP_RING_ITEM_1) || to->IsEquipUniqueItem(NEW_PET_EXP_RING_ITEM_2) || to->IsEquipUniqueItem(NEW_PET_EXP_RING_ITEM_3))
		iPetExp += iPetExp * 50 / 100;
#endif
#ifdef ENABLE_ASLAN_BUFF_NPC_SYSTEM
	int iBuffExp = CALCULATE_VALUE_LVDELTA(to->GetBuffNPCSystem()->GetLevel(), from->GetLevel(), iExp);
	
	if (to->IsBuffEquipUniqueItem(ASLAN_BUFF_EXP_RING_ITEM_1) || to->IsBuffEquipUniqueItem(ASLAN_BUFF_EXP_RING_ITEM_2) || to->IsBuffEquipUniqueItem(ASLAN_BUFF_EXP_RING_ITEM_3))
...
#ifdef ENABLE_ASLAN_GROWTH_PET_SYSTEM
	if (to->GetGrowthPetSystem()) {
		if (to->GetGrowthPetSystem()->IsSummoned() && to->GetGrowthPetSystem()->GetLevelStep() < 4) {
			to->GetGrowthPetSystem()->UpdateExperience(iPetExp, 0);

			if (to->GetGrowthPetSystem()->IsSummoned())
				from->CreateFly(FLY_EXP, to->GetGrowthPetSystem()->GetCharacter());
		}
	}
#endif
#ifdef ENABLE_ASLAN_BUFF_NPC_SYSTEM
...
struct FuncAggregateMonster
{
	...
	void operator()(LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			if (number(1, 100) <= 50)
				if (DISTANCE_APPROX(ch->GetX() - m_ch->GetX(), ch->GetY() - m_ch->GetY()) < 6000)
					if (ch->CanBeginFight())
						ch->BeginFight(m_ch);
		}
	}
