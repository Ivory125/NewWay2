...
ACMD(do_pet_feed_add_list) 
{
	...
	str_to_number(feedslot, arg2);
	str_to_number(invslot, arg3);

	LPITEM item = ch->GetInventoryItem(invslot);

	if (item == NULL)
		return;

	if (ch->GetGrowthPetSystem()->CheckFeedItem(feedtype, item))
		if (feedtype == 3)
			ch->GetGrowthPetSystem()->SetReviveItem(feedslot - 10, invslot);
		else if (feedtype != 3 && ch->GetGrowthPetSystem()->IsSummoned())
			ch->GetGrowthPetSystem()->SetItemFeed(feedslot, invslot);
	else
		return;
}

ACMD(do_pet_feed_eat)
{
	char arg1[256];
	one_argument(argument, arg1, sizeof(arg1));
	...
	if (ch->GetGrowthPetSystem()->IsSummoned())
		ch->GetGrowthPetSystem()->ItemFeed(feedtype);
	else
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ASLAN_GROWTH_PET_SHOULD_SUMMONED"));
}
ACMD(do_pet_revive_special)
{
	char arg1[256];
	one_argument(argument, arg1, sizeof(arg1));
	...
	ch->GetGrowthPetSystem()->RevivePet(item, true);
}

ACMD(do_pet_get_type)
{
	...
	if (ch->GetGrowthPetSystem()->IsSummoned())
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_COMMAND, "GrowthPetGetType %d %d", 1, ch->GetGrowthPetSystem()->GetPetType());
			item->SetCount(item->GetCount() - 1);
		}
		else
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ASLAN_GROWTH_PET_ORB_COUNT_IS_ZERO"));
	}
	else
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ASLAN_GROWTH_PET_SHOULD_SUMMONED"));
	return;
...
ACMD(do_pet_change_attr) 
{
	...
	{
		if (item->GetType() == ITEM_PET_ASLAN && item->GetSubType() == PET_SEAL_ASLAN && item2->GetVnum() == 55033)
		{
			if (item2->GetCount() > 0)
			{
				if (item->GetSocket(1) - time(0) <= 0) {
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ASLAN_GROWTH_PET_MUST_REVIVE"));
					return;
				}

				if (ch->GetGrowthPetSystem()->IsSummoned() && item->GetID() == ch->GetGrowthPetSystem()->GetSummonItemID()) {
					if(ch->GetGrowthPetSystem()->ChangePetAttr())
						item2->SetCount(item2->GetCount() - 1);
				}
				else {
					if (ch->GetGrowthPetSystem()->ChangePetAttr(item))
						item2->SetCount(item2->GetCount() - 1);
				}
			}
			else
				ch->ChatPacket(CHAT_TYPE_INFO, "[PET] Failure change attr");
		}
	}
	return;
}

ACMD(do_pet_remove_skill) 
{
	char arg1[256], arg2[256], arg3[256];
	...
	if (ch->GetGrowthPetSystem()->IsSummoned())
	{
		if (itemvnum == 55029)
		{
			if (ch->GetGrowthPetSystem()->GetPetSkillType(0) >= 1 || ch->GetGrowthPetSystem()->GetPetSkillType(1) >= 1 || ch->GetGrowthPetSystem()->GetPetSkillType(2) >= 1)
			{
				if (ch->CountSpecifyItem(itemvnum) > 0)
				{
					ch->GetGrowthPetSystem()->RemovePetSkill(skillslot, itemvnum);
					ch->RemoveSpecifyItem(itemvnum);
				}
			}
			else
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ASLAN_GROWTH_PET_CANNOT_RESET_ALL_SKILLS"));
		}
		else if (itemvnum == 55030)
		{
			if (skillslot == 0 && ch->GetGrowthPetSystem()->GetPetSkillType(0) >= 1 || skillslot == 1 && ch->GetGrowthPetSystem()->GetPetSkillType(1) >= 1 || skillslot == 2 && ch->GetGrowthPetSystem()->GetPetSkillType(2) >= 1)
			{
				if (ch->CountSpecifyItem(itemvnum) > 0)
				{
					ch->GetGrowthPetSystem()->RemovePetSkill(skillslot, itemvnum);
				...
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ASLAN_GROWTH_PET_CANNOT_RESET_SKILL"));
		}
	}
	else
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ASLAN_GROWTH_PET_SHOULD_SUMMONED"));
...
ACMD(do_pet_evo_open)
{
	if (ch->GetExchange() || ch->GetMyShop() || ch->GetShopOwner() || ch->IsOpenSafebox() || ch->IsCubeOpen()) {
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ASLAN_GROWTH_PET_CANNOT_EVOLVE_NOW"));
		return;
	}
	if (ch->GetGrowthPetSystem()->IsSummoned()) {
		BYTE isEvo = ch->GetGrowthPetSystem()->GetEvolution();

		if (isEvo >= GROWTH_PET_MAX_EVO) {
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ASLAN_GROWTH_PET_IS_MAX_EVO"));
			return;
		}

		if (ch->GetGrowthPetSystem()->GetLevel() >= PetEvoTable[isEvo].dwLevel) {
			int tableSize = (sizeof PetEvoTable[isEvo].dwItemVnum / sizeof PetEvoTable[isEvo].dwItemVnum[0]);
			for (int i = 0; i < tableSize; ++i) {
				if (PetEvoTable[isEvo].dwItemVnum[i] != 0 && PetEvoTable[isEvo].dwItemVnum[i] != -1)
					ch->ChatPacket(CHAT_TYPE_COMMAND, "GrowthPetSetEvoInfo %d %d %d", 0, PetEvoTable[isEvo].dwItemVnum[i], PetEvoTable[isEvo].dwItemCount[i]);
			}
			ch->ChatPacket(CHAT_TYPE_COMMAND, "GrowthPetSetEvoInfo %d %d %d", 1, 0, 0);
		}
		else {
			ch->ChatPacket(CHAT_TYPE_INFO, "[PET] Failure open evo info");
			return;
		}
...
ACMD(do_pet_evo_upgrade)
{
	...
	if (ch->GetGrowthPetSystem()->IsSummoned()) {
		...
		if (ch->GetGrowthPetSystem()->CanEvolution())
			ch->GetGrowthPetSystem()->IncreasePetEvolution();
		else {
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ASLAN_GROWTH_PET_CANNOT_EVOLVE_NOW"));
			return;
		}
