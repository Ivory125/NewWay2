...
ACMD(do_pet_set_level)
{
	...
	if (!*arg1) {
		ch->ChatPacket(CHAT_TYPE_INFO, "Syntax: pet_set_level <level>");
		return;
	}

	int	level = 0;
	str_to_number(level, arg1);
	level = MINMAX(1, level, GROWTH_PET_MAX_LEVEL);

	if (ch->GetGrowthPetSystem()->IsSummoned()) {
		ch->GetGrowthPetSystem()->SetLevel(level);
		ch->GetGrowthPetSystem()->ChangePetAttr();
	}
	else
		ch->ChatPacket(CHAT_TYPE_INFO, "[CMD-GM | PET] Summon a Pet");
}
...
ACMD(do_pet_set_evolution)
{
	...
	int	evo = 0;
	str_to_number(evo, arg1);
	evo = MINMAX(0, evo, 3);

	if (ch->GetGrowthPetSystem()->IsSummoned()) {
		ch->GetGrowthPetSystem()->SetEvolution(evo);
	}
	else 
		ch->ChatPacket(CHAT_TYPE_INFO, "[CMD-GM | PET] Summon a Pet");
}
...
ACMD(do_pet_set_exp)
{
	...
	if (ch->GetGrowthPetSystem()->IsSummoned())
		ch->GetGrowthPetSystem()->UpdateExperience(experience, type);
	else
		ch->ChatPacket(CHAT_TYPE_INFO, "[CMD-GM | PET] Summon a Pet");
}
#endif
