...
static const std::set<int> ALLOWED_SPAWN_MAPS = { 656 };
...
namespace ikashop
{
	...
	bool CShopManager::RecvShopAddItemClientPacket(LPCHARACTER ch, const TItemPos& pos, const TPriceInfo& price, int destPos)
	{
		...
		sitem.grade_element = item->GetElementGrade();
		thecore_memcpy(sitem.attack_element, item->GetElementAttacks(), sizeof(sitem.attack_element));
		sitem.element_type_bonus = item->GetElementsType();
		thecore_memcpy(sitem.elements_value_bonus, item->GetElementsValues(), sizeof(sitem.elements_value_bonus));
#endif
...
#endif
#ifdef ELEMENT_SPELL_WORLDARD
		auction.grade_element = item->GetElementGrade();
		thecore_memcpy(auction.attack_element, item->GetElementAttacks(), sizeof(auction.attack_element));
		auction.element_type_bonus = item->GetElementsType();
		thecore_memcpy(auction.elements_value_bonus, item->GetElementsValues(), sizeof(auction.elements_value_bonus));
#endif
