...
CItem::CItem(DWORD dwVnum)
	...
	m_pkTimerCheckPetDeadEvent(NULL),
#endif
...
#ifdef ENABLE_ASLAN_GROWTH_PET_SYSTEM
	event_cancel(&m_pkTimerCheckPetDeadEvent);
#endif
...
int CItem::FindBuffEquipCell(LPCHARACTER ch, int iCandidateCell)
{
	...
	else if (GetType() == ITEM_SKILLBOOK && GetSocket(0) >= 94 && GetSocket(0) <= 96 || GetType() == ITEM_SKILLBOOK && GetSocket(0) >= 109 && GetSocket(0) <= 111|| GetVnum() >= 50494 && GetVnum() <= 50496 || GetVnum() >= 50509 && GetVnum() <= 50511 || GetVnum() == 50513)
...
#ifdef ENABLE_ASLAN_GROWTH_PET_SYSTEM
EVENTFUNC(timer_check_pet_dead_event)
{
	item_event_info* info = dynamic_cast<item_event_info*>(event->info);

	if (info == NULL) {
		sys_err("pet_dead_event <Factor> Null pointer");
		return 0;
	}
	...
	if (pkItem->GetSocket(1) <= time(0)) {
		sys_log(0, "ITEM PET DEAD : dead %s %u", pkItem->GetName(), pkItem->GetID());
		pkItem->SetTimerCheckPetDeadEvent(NULL);
		pkItem->SetSocket(4, time(0));
		return 0;
	}
	return PASSES_PER_SEC(60);
}
...
#ifdef ENABLE_ASLAN_GROWTH_PET_SYSTEM
void CItem::SetTimerCheckPetDeadEvent(LPEVENT pkEvent)
{
	m_pkTimerCheckPetDeadEvent = pkEvent;
}
#endif
...
#ifdef ENABLE_ASLAN_GROWTH_PET_SYSTEM
void CItem::StartCheckPetDeadEvent()
{
	if (m_pkTimerCheckPetDeadEvent)
		return;

	item_event_info* info = AllocEventInfo<item_event_info>();
	info->item = this;

	SetTimerCheckPetDeadEvent(event_create(timer_check_pet_dead_event, info, PASSES_PER_SEC(60)));
}
void CItem::StopCheckPetDeadEvent()
{
	if (!m_pkTimerCheckPetDeadEvent)
		return;
