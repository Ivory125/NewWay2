...
enum
{
	...
	HEADER_CG_PET_BORN					= 116,
	HEADER_CG_PET_RENEW_NAME		= 117,
#endif
...
#ifdef ENABLE_SHOW_CHEST_DROP // Überprüfung, ob die Anzeige der Truhen-Drops aktiviert ist

// Definition von Header-IDs für die Client-Server-Kommunikation
enum
{
	HEADER_CG_CHEST_DROP_INFO = 215, // Header-ID für die Client-an-Server-Truhen-Drop-Informationen
	HEADER_GC_CHEST_DROP_INFO = 219, // Header-ID für die Server-an-Client-Truhen-Drop-Informationen
};

// Definition der Struktur für die Client-Truhen-Drop-Informationen
typedef struct SPacketCGChestDropInfo {
	BYTE	header; // Header der Nachricht
	WORD	wInventoryCell; // Zielzelle im Inventar
} TPacketCGChestDropInfo;

// Definition der Struktur für die Truhen-Drop-Informationen
typedef struct SChestDropInfoTable {
	BYTE	bPageIndex; // Index der Seite in der Truhe
	BYTE	bSlotIndex; // Index des Slots in der Truhe
	DWORD	dwItemVnum; // Vnum des Items
	BYTE	bItemCount; // Anzahl des Items
} TChestDropInfoTable;

// Definition der Struktur für die Server-Truhen-Drop-Informationen
typedef struct SPacketGCChestDropInfo {
	BYTE	bHeader; // Header der Nachricht
	WORD	wSize; // Größe der Nachricht
	DWORD	dwChestVnum; // Vnum der Truhe
...
#ifdef ENABLE_ASLAN_GROWTH_PET_SYSTEM
typedef struct SPacketGCPetInfoPacket
{
	BYTE	bHeader;
	DWORD	dwSealItemVnum;
	WORD	dwSealSlot;
	char	szPetName[13];
	DWORD	dwAge;
	DWORD	dwBirthday;
	DWORD	dwItemDuration;
	DWORD	dwBornDuration;
	DWORD	dwLevel;
	DWORD	dwEvo;
	bool	bCanEvolution;
	DWORD	dwExpMob;
	...
	DWORD	dwNextExp[2];
	BYTE	dwBonus[3];
	DWORD	dwSkillInfo[3][2];
} TGrowthPetInfoPacket;

typedef struct SPacketCGBornPet
{
	BYTE byHeader;
	char petname[13];
} TPacketCGBornPet;

typedef struct SPacketCGRenewPetName
{
	BYTE byHeader;
	char petname[13];
	int itempos1;
	int itempos2;
}TPacketCGRenewPetName;
#endif
